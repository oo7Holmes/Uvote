...
public class votes extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String vid=mdata1.child(firebaseAuth.getCurrentUser().getUid()).toString();

        mdata1.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                long size = dataSnapshot.child(firebaseAuth.getCurrentUser().getUid()).child(vid).getChildrenCount();
        ...
        mdata2.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                long size1 = dataSnapshot.child("Rahul Gandhi").getChildrenCount();
        ...
        mdata3.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                long size2 = dataSnapshot.child("Akhilesh Singh Yadav").getChildrenCount();
        ...
        mdata4.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                long size3 = dataSnapshot.child("Yogi Adityanath").getChildrenCount();
